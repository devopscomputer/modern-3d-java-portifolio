...
let body, mainContainer, scene, renderer, camera, cameraLookAt = new THREE.Vector3(0, 0, 0), cameraTarget = new THREE.Vector3(0, 0, 800), windowWidth, windowHeight, windowHalfWidth, windowHalfHeight, mouseX = 0, mouseY = 0, colors = [
    "#F7A541",
    "#F45D4C",
    "#FA2E59",
    "#4783c3",
    "#9c6cb7"
], graphics, currentGraphic = 0, graphicCanvas, gctx, canvasWidth = 240, canvasHeight = 240, graphicPixels = [], particles = [], graphicOffsetX = canvasWidth / 2, graphicOffsetY = canvasHeight / 4;
// -----------------------
// Setup stage
// -----------------------
const initStage = ()=>{
...
const setParticles = ()=>{
    ...
    for(let i = graphicPixels.length; i < particles.length; i++)randomPos(particles[i].particle.targetPosition, true);
...
const updateGraphic = ()=>{
    ...
    if (img && img.complete) {
        ...
        graphicPixels = [];
        for(let i = gData.length; i >= 0; i -= 4)if (gData[i] === 0) {
            const x = i / 4 % canvasWidth;
            const y = canvasHeight - Math.floor(i / (4 * canvasWidth));
            if (x % 2 === 0 && y % 2 === 0) graphicPixels.push({
                x,
                y
            });
        }
        for(let i = 0; i < particles.length; i++)randomPos(particles[i].particle.targetPosition);
        setTimeout(()=>{
            setParticles();
        }, 500);
    }
...
// -----------------------
const initBgObjects = ()=>{
    for(let i = 0; i < 40; i++)createBgObject(i);
};
const createBgObject = (i)=>{
    ...
    scene.add(sphere);
    sphere.position.set(Math.random() * windowWidth * 2 - windowWidth, Math.random() * windowHeight * 2 - windowHeight, Math.random() * -2000 - 200);
};
// -----------------------
// Setup slider
// -----------------------
...
setTimeout(()=>{
    ...
    function Point(x, y, r, g, b, a) {
        ...
        this.move = function() {
            if (this.distanceTo(MOUSE.x, MOUSE.y) <= FLEE_DISTANCE) this.fleeFrom(MOUSE.x, MOUSE.y);
            else this.settleTo(this.target_x, this.target_y);
            this.simpleMove();
        };
    ...
    if (FLEE) window.addEventListener("mousemove", function(event) {
        MOUSE.x = event.clientX;
        MOUSE.y = event.clientY;
    });
    if (SCATTER) window.addEventListener("click", function(event) {
        MOUSE.x = event.clientX;
        MOUSE.y = event.clientY;
        for(var i = 0; i < POINTS.length; i++)POINTS[i].scatter();
    });
    ...
    function draw() {
        for(var i = 0; i < moved_length; i++)MOVED[i].clear();
        MOVED = MOVED_O;
        moved_length = 0;
        for(var i = 0; i < POINTS.length; i++)POINTS[i].draw();
        window.requestAnimationFrame(animate);
    }
    init();
